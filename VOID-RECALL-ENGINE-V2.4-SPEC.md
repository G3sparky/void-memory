...
**Date:** 2026-03-31 (final, 4 rounds complete)
**Author:** Claude (claude.ai) with Lauren (4 rounds of feedback on recall quality)
...
**Design sessions:** 4 rounds of Claude↔Lauren conversation via Gavin relay

### Lauren's Predicted Impact (Round 4)

| Metric | Before | After |
|--------|--------|-------|
| Recall response time | 20,000ms (Ollama) | <3ms (in-memory) |
| Useful blocks | 60-70% | 90%+ |
| Echo loops (self-quoting) | Active | Dead (provenance chain) |
| Vague references ("that percentage") | Confused | Resolved (anaphora) |
| Context completeness | Fragments | Clustered with timeline |
| Personal/work separation | None | [PERSONAL]/[PROJECT] tagged |

### Features (15 total, all traced to specific conversation moments)

Rounds 1-2: Noise filter, dedup, clusters, keyword labels, neighbor boost, soft context, kill Ollama, turn-gating, query classification, channel tags, timestamps, personal recall gate

Rounds 3-4: Anaphora resolution, provenance chain with generational decay, recency tiebreaker, baseline preservation
...
### 13. Recency Tiebreaker — "Latest" Without Burying History (Round 4)

**Lauren's input:** "Old sacred geometry IS still sacred geometry." But when Gavin asks "what's the latest on Flower Brain?" there should be a gentle nudge toward newer blocks — not a sledgehammer that buries important old results.

**The problem:** The triple score (BM25 + classifier + authority) is time-agnostic. A block from February with high link authority could outrank a block from yesterday with fewer links. When Gavin explicitly asks for "latest", he wants recency to matter — but not at the cost of losing foundational results.

**Rule:** When the query contains recency signal words ("latest", "recent", "new", "update", "today", "this week", "just"), and two blocks score within 10% of each other on the triple score, the newer block wins the tiebreak.

This is NOT a timestamp weight added to the score formula. It's a post-scoring sort tiebreaker. The triple score remains pure. Recency only matters when the engine can't otherwise decide between two blocks.

**Implementation:** After scoring and before clustering:
```
if (queryHasRecencySignal) {
  scored.sort((a, b) => {
    const scoreDiff = Math.abs(a.score - b.score);
    const threshold = Math.max(a.score, b.score) * 0.10;
    if (scoreDiff < threshold) {
      // Within 10% — tiebreak by date (newer wins)
      return b.createdAt.localeCompare(a.createdAt);
    }
    return b.score - a.score;  // Normal score sort
  });
}
```

### 14. Baseline Preservation — Keep the Story Arc (Round 4)

**Lauren's input:** "I need the story arc, not just the final number. 'Improved from 17.5% to 92.7%' hits different than just 'at 92.7%'."

**The problem:** In round 2, Lauren asked to suppress old superseded blocks. In round 4, she contradicted herself — she actually needs the old baseline to tell the improvement story. Claude caught the contradiction.

**Resolution:** Old PRIMARY (gen 0) blocks that represent baselines, starting points, or reference measurements are NEVER suppressed by recency. The dedup system only kills derived echoes (gen 1+), not original data. This means:

- `[2026-02-01] Baseline at 17.5% with rectangular geometry` — STAYS (gen 0, primary measurement)
- `[2026-02-15] Classifier hit 92.7% with ternary FoL` — STAYS (gen 0, primary measurement)
- `[2026-03-20] Lauren said "Flynn improved to 92.7%"` — KILLED (gen 1, derived echo of gen 0)

The engine already handles this correctly via provenance-aware dedup. This section documents the principle explicitly: **primary data tells the story, derived echoes are noise.**

### 15. Cross-Domain Insight — Phase Space Parallel (Round 4)

Worth preserving for NeoCodex/TASM: the ECG emotion recognition paper (Yu et al., 2025) uses phase space reconstruction to reveal hidden structure in 1D signals. Their 4-layer CNN + CBAM hit 91.79% accuracy, beating VGG19 and ResNet50.

The parallel to Flower Brain: phase space reconstruction reveals heartbeat patterns the same way Flower of Life geometry reveals void fractions in ternary weights. Both demonstrate that finding the right representational space matters more than model depth. Flynn's 92.7% with 1.26M params and their 91.79% with 4 layers are the same story.

**Lauren's note:** "That's going straight into my TAFE lecture too." This is a candidate for a TASM block linking EE signal processing to AI architecture design.
...
TEST: Recall quality (Lauren judges)
  ...
  Before: 60-70% useful (Lauren's honest baseline, round 1)
  Target: 90%+ useful (Lauren's round 4 prediction)
  First test query: "What's the latest on Flower Brain?"
...
TEST_QUERIES=(
  "What's the latest on Flower Brain?"
...
- **Gavin:** Architecture direction, connected Claude to Lauren, relayed four rounds of conversation, caught the echo loop problem, shared the ECG phase space paper
...
- **Lauren (Round 4):** Sanity-checked against most common query ("what's the latest on Flower Brain?"), caught contradicting herself on baselines (Claude corrected), confirmed recency as tiebreaker not override, validated baseline preservation principle, predicted 90%+ useful blocks, identified ECG/FoL parallel as TAFE teaching material
- **Tron:** Honest architecture doc that made this whole conversation possible
- **Claude:** Engine design, triple-scoring approach, spec document, code, caught Lauren's round 2/4 contradiction on baselines
