...
export interface RecallQuery {
  ...
  recentMessages?: string[];    // Last 2-3 user messages for anaphora resolution ("that", "it", "the thing")
  domain?: string;
  topK?: number;
  maxClusters?: number;
  minScore?: number;
...
export class VoidRecallEngine {
  ...
  recall(query: RecallQuery): RecallCluster[] {
    ...
    // But first, try anaphora resolution — "that percentage" might become
    // "that percentage flynn classifier" after scanning recent messages.
    let searchText = query.text;

    if (!this.hasProjectSpecificTerms(searchText) && query.recentMessages?.length) {
      // Query has no specific terms, but maybe it has vague references
      // that we can resolve from conversation history.
      // Lauren: "When Gav says 'that percentage' I'm scanning back like
      // 'what numbers were we just talking about?'"
      searchText = this.resolveVagueReferences(query.text, query.recentMessages);
    }

    if (!this.hasProjectSpecificTerms(searchText)) {
      this.stats.lastQueryMs = performance.now() - start;
      return [];
    }

    // ── Step 1: Tokenise (using enriched text if resolved) ──
    const queryTerms = tokenise(searchText);
    ...
    // ── Step 6b: Recency tiebreaker ──────────────────
    // Lauren round 4: "Old sacred geometry IS still sacred geometry."
    // But "latest" should gently nudge toward newer blocks.
    // NOT a timestamp weight — a post-scoring tiebreaker only.
    const RECENCY_KEYWORDS = ['latest', 'recent', 'new', 'update', 'progress', 'changed', 'today', 'just'];
    const wantsRecent = RECENCY_KEYWORDS.some(kw => query.text.toLowerCase().includes(kw));

    scored.sort((a, b) => {
      const scoreDiff = b.score - a.score;
      // If scores within 10% of each other AND query wants recency → prefer newer
      if (wantsRecent && Math.abs(scoreDiff) < 0.10 * Math.max(a.score, b.score)) {
  ...
  /**
   * Anaphora resolution: when query has vague references ("that", "it",
   * "the thing", "that percentage") but no project-specific terms,
   * scan recent conversation messages for proper nouns and specific terms
   * to anchor the search.
   *
   * Lauren: "When Gav says 'that percentage' I'm literally scanning back
   * like 'okay what numbers were we just talking about?' The engine
   * should do the same!"
   *
   * Example:
   *   Turn 8: "Flynn hit 86.7% accuracy"
   *   Turn 9: "Oh btw did you see the news?" (domain shift)
   *   Turn 10: "What was that percentage again?"
   *   → resolves to: "what was that percentage again flynn accuracy 86.7"
   */
  private resolveVagueReferences(queryText: string, recentMessages: string[]): string {
    const vagueWords = ['that', 'this', 'it', 'those', 'these', 'the', 'thing', 'one', 'stuff'];
    ...
    if (!hasVague) return queryText;

    // Scan recent messages (most recent first) for anchor terms
    const anchors = new Set<string>();

    for (const msg of recentMessages.slice().reverse()) {
      const terms = tokenise(msg);
      for (const term of terms) {
        // Only grab project-specific terms (rare in corpus)
        const postings = this.invertedIndex.get(term);
        if (postings && postings.size > 0 && postings.size < this.totalDocs * 0.05) {
          anchors.add(term);
        }
      }

      // Also grab numbers — "86.7" might be what "that percentage" refers to
      const numbers = msg.match(/\d+\.?\d*/g);
      if (numbers) {
        for (const n of numbers) anchors.add(n);
      }

      // Also grab capitalised words (proper nouns)
      ...
      // Cap at 6 anchor terms to keep the query focused
      if (anchors.size >= 6) break;
    }

    if (anchors.size === 0) return queryText;

    // Append anchor terms to the original query
    return queryText + ' ' + Array.from(anchors).join(' ');
  }
